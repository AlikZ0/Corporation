...
type User = { id: string; email: string; passwordHash: string; avatarDataUrl?: string };
...
app.patch('/api/me', authMiddleware, async (req, res) => {
  ...
  const user = users.get(id);
  if (!user) return res.status(404).json({ message: 'Not found' });
  const { email, avatar } = req.body as { email?: string; avatar?: string | null };
  if (email) {
  ...
  if (avatar !== undefined) {
    if (avatar === null) user.avatarDataUrl = undefined;
    else if (typeof avatar === 'string' && avatar.startsWith('data:')) user.avatarDataUrl = avatar;
    else return res.status(400).json({ message: 'Invalid avatar' });
  }
  users.set(id, user);
